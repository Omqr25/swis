...
export const BranchForm = ({ isEdit , ID }: Props) => {
  ...
  const validationsEditBranch = yup
    ...
    .shape({
      ...
      function (value) {
        ...
        const atLeastOneFieldHasValue = !!name|| !!phone || !!address || !!parent_id;
    ...
    const validationsAddBranch = yup
    .object()
    .shape({
  ...
  const Edit = useEditBranch(ID);
  const Create = useCreateBranch();
  const branch = useBranche(ID);
  ...
  if(isEdit && branch.isLoading)return <Text color={'white'}>Loading...</Text>
  return (
    <Formik
      initialValues={{
